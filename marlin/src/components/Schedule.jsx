      ...
      <Card title="Shop load by month" icon={Gauge}>
        <div className="p-4">
            ...
            {SHOP_LOAD.map(({ month: m, load: v }) => {
              ...
              return (
                <div key={m} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-md relative" style={{ height: `${v}%`, background: peak ? "#b42121" : trough ? "#1aa0c4" : "#9fb4bd" }}>
          ...
          <div className="flex gap-4 mt-4 text-[12px]">
            ...
            <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded bg-[#1aa0c4]" /> Dead trough, sell into it</span>
          </div>
        </div>
      </Card>
      <div className="mt-4">
        <Card title="Suggested actions to level the curve" icon={ListChecks}>
          {SCHEDULE_ACTIONS.map((a, i) => (
            <div key={a.id} className="flex gap-3 px-4 py-3 border-b border-[#f4f4f4] last:border-0">
              <span className="w-6 h-6 rounded-full bg-[#e9f5f9] text-[#0a6e8c] grid place-items-center text-xs font-bold shrink-0">{i + 1}</span>
              <div>
