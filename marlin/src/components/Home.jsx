  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {kpis.map((k) => (
          <div key={k.label} className="bg-white rounded-lg border border-[#e5e5e5] shadow-sm p-4">
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2">
          <Card title="Boats due soon" icon={Ship} action={<span className="text-xs text-[#1aa0c4] font-semibold">{dueSoon.length} records</span>}>
            {dueSoon.map((b) => (
              <button key={b.id} onClick={() => openRecord(b.id)} className="w-full flex items-center gap-3 px-4 py-3 border-b border-[#f4f4f4] hover:bg-[#f7fbfd] text-left">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-[#0a6e8c] truncate">"{b.name}"</div>
              ...
              {byBoat.map((r) => (
                <button key={r.boatId} onClick={() => openRecord(r.boatId)} className="w-full flex items-center gap-3 px-4 py-2.5 border-b border-[#f4f4f4] hover:bg-[#f7fbfd] text-left">
                  <div className="flex-1">
                    <span className="font-semibold text-[#0a6e8c]">"{boat(r.boatId).name}"</span>
        ...
        <Card title="Recent activity" icon={Clock}>
          {RECENT_ACTIVITY.map((a) => (
            <div key={a.id} className="flex gap-3 px-4 py-3 border-b border-[#f4f4f4]">
              <span className="w-2 h-2 rounded-full bg-[#1aa0c4] mt-1.5 shrink-0" />
