  ...
  return (
    <div className="bg-white rounded-lg border border-[#dddbda] shadow-sm overflow-hidden">
      <div className="px-4 py-2.5 border-b border-[#f0f0f0] flex items-center justify-between">
        ...
        <Pill bg="#e9f5f9" fg="#0176d3">{job.tech.split(" ")[0]} on it</Pill>
      </div>
      <div className="p-4">
            ...
            return (
              <div key={s} className="flex items-center gap-2.5 py-1">
                {done ? <CheckCircle2 size={18} className="text-[#1b6b34]" />
                  : current ? <Clock size={18} className="text-[#0176d3]" />
                    : <Circle size={18} className="text-[#d0d4d8]" />}
                <span className={"text-[13px] " + (current ? "font-bold text-[#0176d3]" : done ? "text-[#3a3a3a]" : "text-[#9aa0a6]")}>{s}</span>
        ...
        <div className="mt-3 flex items-center gap-2 text-[12px] text-[#5f6368] bg-[#f7fbfd] rounded-md p-2">
          <Camera size={14} className="text-[#0176d3]" />
          Owner sees each step by SMS. Tap, photo, dictate. AI writes the message.
        </div>
        {job.stage < STAGES.length - 1 && (
          <button onClick={() => advance(job.id)} className="mt-3 w-full bg-[#16325c] hover:bg-[#0b2447] text-white text-sm font-semibold rounded-md h-9">
            Advance stage
          </button>
        )}
        {job.stage === STAGES.length - 1 &&
